...
public class Client implements ActionListener, ChangeListener {
    ...
    private static String jarLocation = System.getProperty("user.home") + File.separator + "alora" + File.separator + "client.jar";
    ...
    private void initApplet() throws Exception {
        ...
        classLoader =  AppletClassLoader.newInstance(new URL[]{new File(jarLocation).toURI().toURL()});
        final Class<?> loader = classLoader.loadClass("ClientLoader");
        clientApplet = (Applet) loader.getDeclaredConstructor(String.class, boolean.class).newInstance("1", true);
        ...
        /*
         * find the static client loader field and set it to the new
         * instance we just created, so it is no longer null *for security purposes*
         */
        for (Field field : loader.getDeclaredFields()) {

            if (Modifier.isStatic(field.getModifiers())) {

                if (field.getType().isAssignableFrom(loader)) {

                    Main.debug("Identified static alora Client Loader field as " + field.getName());
                    field.setAccessible(true);

                    if (field.get(null) == null) {
                        Main.debug("Setting null Client loader field to new applet instance");
                        field.set(null, clientApplet);
                    }
                } else if (field.getType().isAssignableFrom(JFrame.class)) {

                    Main.debug("Found static JFrame field " + field.getName());
        ...
        /*
         * dispose of Alora's JFrame
         */
        if (frameField != null) {
            ((JFrame) frameField.get(null)).dispose();
        }
